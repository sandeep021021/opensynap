...
def wrap_note_message(message: str, *, max_width: Optional[int] = None, columns: Optional[int] = None) -> str:
    ...
    # Clack-like: slightly narrower than full width; avoid huge boxes
...
def note(message: str, title: Optional[str] = None) -> None:
    """
    Clack-ish note panel (Rich tuned):
    - Rounded border, but dim
    - Small padding
    - Title styled subtly
    - Body dim
    """
    wrapped = wrap_note_message(message)
    console = Console()

    if not is_rich():
        # Plain fallback
        print()
        print(f"ℹ {title or 'Note'}")
        for line in wrapped.split("\n"):
            print(f"  {line}")
        print()
        return

    panel_title = Text(title or "Note", style="bold")  # clack-like label (subtle)
    body = Text(wrapped, style="dim")
    ...
    console.print(
        Panel(
            ...
            title_align="left",
            box=box.ROUNDED,
            padding=(0, 2),       # tighter than before
            border_style="dim",   # soft border like clack
        )
    )